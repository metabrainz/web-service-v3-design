...
This version of the MusicBrainz API uses a new rate limiter, which is different
to the rate limiter that runs on previous versions. Rather than imposing a fixed
rate of API calls, the new rate limiter gives clients permission to make a
variable amount of calls every hour, and imposes no limit on how fast these
calls can be made.

When a client makes a request, the rate limiter first checks to see if the
client has made any calls within the last hour. If so, it will check that the
client still has permission to make more calls and fail the request if
not. Otherwise, the rate limiter will decrement the client's counter and pass
the call to the API. The final response will include 2 HTTP headers:

* `RateLimit-Remaining`: a non-negative integer of the amount of calls the
  client can make in their current rate limited period.
* `RateLimit-Refresh`: The amount of time the client has to wait until their
  rate limit balance will be refreshed.

### Variable Rate Limit

The amount of calls you can make an hour will vary depending on MusicBrainz
server load. If MusicBrainz is currently serving a lot of requests, then your
hourly allowance will be lowered to allow all requests to be served. However,
whatever number we've given in a rate limit period will roll over to your next
rate limit period, as long as you don't let an hour pass between calls. For
example, if we gave you 3000 requests, and then lowered the rate limit to 2000,
you would still be allowed to make 3000 requests.
